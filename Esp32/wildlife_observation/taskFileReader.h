...
/* new task structure*/
// Our task is minuate based --> 24hr == 1440min --> if use char : only 1440
// byte We can simply fill the array by task like ["0", "1", "2", "0", ...
// ,"0"]. Every slot is a min About the record setting, we can use another array
...
// recordSetting *recordSettingArray = (recordSetting*)malloc(
// sizeof(recordSetting) * recordSettingArrayMaxSize);

/*-------- function implement --------*/
int hour24ConvetToMin(int input) { return (input / 100) * 60 + input % 100; }

int minConvertTohour24(int input) { return (input / 60) * 100 + input % 60; }
...
#define uzipTaskLength 5
#define TASK_R 0
#define TASK_A 1
#define TASK_B 2
#define TASK_C 3
#define TASK_D 4  // reboot
uint8_t *uint8NumToCharArr(uint8_t input) {
    // [R,A,B,C,D]
    uint8_t *taskArray = (uint8_t *)malloc(sizeof(uint8_t) * uzipTaskLength);
    // R
    taskArray[TASK_R] = input / 16;
    input = input % 16;
    // A
...
String uint8ToString(uint8_t input) {
    ...
    String rtn = "";
    char tasks[] = {'D', 'C', 'B', 'A', 'R'};
    for (int i = 0; i < uzipTaskLength; ++i) {
        // i++ and ++i is the same in this case, but in compiler level, ++i is
        // faster input compare with the bit mask, 1<<i is 2^i
        if (input & (1 << i)) {
...
void addOneSetting(recordSetting **pointerToTaskArray,
                   recordSetting *pointerToTask, int *pointerToArrayMaxSize,
                   int *pointerToArrayUsedIndex) {
    // if array size is not enough
    if (*pointerToArrayUsedIndex == *pointerToArrayMaxSize) {
        // create a temp array with double size
        recordSetting *tempTaskArray = (recordSetting *)malloc(
            sizeof(recordSetting) * *pointerToArrayMaxSize * 2);
        // copy data to new array
        memcpy(tempTaskArray, *pointerToTaskArray,
               *pointerToArrayMaxSize * sizeof(recordSetting));
...
void addTask(String input) {
    ...
    char
        *temp[5];  // a pointer can storage 5 char array, meaning : [start_time,
                   // task, duration/period, L/R(if have), ratial(if have)]

    //// parse that line of input (split by ",")
    temp[index] =
        strtok((char *)input.c_str(),
               ",");  // cut the input by "," and return the first cut part
    while (temp[index] != NULL) {  // if still can split, keep running
        ...
        temp[index] = strtok(
            NULL,
            ",");  // keep split from the last position by passing NULL into it.
    }
#ifdef PARSE_TASK_DEBUG
    Serial.println("");
#endif
    ...
    if (index == 3) {
        ...
        addOneSetting(&recordSettingArray, &oneTempSetting,
                      &recordSettingArrayMaxSize, &recordSettingArrayUsedSize);
    }
}
...
void checkScheduleFileExist() {
    if (!sd.exists(SCHEDULE_FILE.c_str())) {
        ...
        writeMsgToPath(systemLogPath,
                       "Init failed! Don't have file : " + SCHEDULE_FILE +
                           ". Please see exampleSchedule.txt");
        ...
        writeMsgToPath("example_schedule.txt",
                       ...
                       "參數說明 : \n"
                       "任務 A\n"
                       "初始時間 (24小時制,無標點,不可有小數點), 任務代碼, "
                       "執行時間(sec,不可有小數點), L/R/B (聲道左/右/兩者), "
                       "音量幾倍 (基準為1，建議範圍 : 0.5 ~ 2倍)\n"
                       "任務 BCD\n"
                       "初始時間 (24小時制,無標點,不可有小數點), 任務代碼, "
                       "執行間隔(min,不可有小數點)\n"
                       "\n"
                       "其他 : \n"
                       "請將 example_schedule.txt 重新命名成 "
                       "schedule.txt，程式才能正確執行\n"
                       "\n"
                       "常用換算 : \n"
                       "01 min = 60 sec\n"
                       "05 min = 300 sec\n"
                       ...
                       "60 min = 3600 sec\n",
                       "", true,
                       false);  // custem timestamp msg, append, timestamp
...
//// info
    ...
    while (taskFile.available()) {
        ...
        while (1) {
            char tempChar =
                taskFile.read();  // read a word, then save to buffer
            if (tempChar == '\n') {
                break;
            }
...
// all reboot will just after task A
void addRebootTasks() {
    // read task list
    for (int i = 0; i < MIN_A_DAY; i++) {
        // find the task A (and it's not out of range)
        if (taskScheduleList[i] & 8 && i + 1 < MIN_A_DAY) {
            // add reboot task after task A
            taskScheduleList[i + 1] = taskScheduleList[i + 1] & 16;
        }
    }
}
...
void findTheIndexOfCurrentTime() {
    ...
    // 23:59:00 ~ 23:59:59 == 1439min00sec~59sec --> readTaskIndex == 1440, but
    // out of range so it need to re-zero
    ...
    for (int i = 0; i < MIN_A_DAY; i++) {
        ...
        if (taskScheduleList[i] != 0) {
            Serial.println("Index : " + String(i) +
                           ", task : " + String(taskScheduleList[i]) + "(" +
                           uint8ToString(taskScheduleList[i]) + ")");
            // if have record task
            if (taskScheduleList[i] > 7) {
                Serial.println(
                    "-- duration_time : " +
                    String(recordSettingArray[settingIndex].duration_time));
                Serial.println(
                    "-- channel : " +
                    String(recordSettingArray[settingIndex].channel));
                Serial.println(
                    "-- multiple : " +
                    String(recordSettingArray[settingIndex].multiple));
                settingIndex += 1;
            }
        }
    }
