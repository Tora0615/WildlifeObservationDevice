...
void createRTOSTasks() {
    ...
    xTaskCreatePinnedToCore(showTaskRunningLED,   /* Task function. */
    ...
    xTaskCreatePinnedToCore(showLowBattery,          /* Task function. */
    ...
    xTaskCreatePinnedToCore(checkGoSleep,          /* Task function. */
    ...
    xTaskCreatePinnedToCore(checkTimeAndTask,   /* Task function. */
    ...
    xTaskCreatePinnedToCore(transmitSoundDataToSD,   /* Task function. */
    ...
    xTaskCreatePinnedToCore(recordSound,          /* Task function. */
    ...
    xTaskCreatePinnedToCore(recordDHT,          /* Task function. */
    ...
    xTaskCreatePinnedToCore(recordDS18B20,          /* Task function. */
    ...
    xTaskCreatePinnedToCore(recordBattery,          /* Task function. */
...
// When have any error, set flag to true, then use highest task to block all
// tasks.
...
void showTaskRunningLED(void *pvParameters) {
    ...
    while (true) {
        ...
        Serial.println("#### showTaskRunningLED stack : " +
                       String(uxHighWaterMark) + "(byte) ####");
        writeMsgToPath(systemLogPath, "#### showTaskRunningLED stack : " +
                                          String(uxHighWaterMark) +
                                          "(byte) ####");
#endif
    }
}
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### showLowBattery stack : " +
                       String(uxHighWaterMark) + "(byte) ####");
        writeMsgToPath(systemLogPath, "#### showLowBattery stack : " +
                                          String(uxHighWaterMark) +
                                          "(byte) ####");
#endif
    }
...
// if called resume, will go to here
        ...
        Serial.print("- isRunningTask : " + String(isRunningTask));
        Serial.println(", previousRoundOfSleepFinished : " +
                       String(previousRoundOfSleepFinished) + "\n");
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### checkGoSleep stack : " + String(uxHighWaterMark) +
                       "(byte) ####");
        writeMsgToPath(systemLogPath,
                       "#### checkGoSleep stack : " + String(uxHighWaterMark) +
                           "(byte) ####");
#endif
    }
}
...
// Debug msg
        ...
        /*
          ...
          min
          | 0 | 1 | 2 | 3 | ... | 1439 |  (A day 1440 min, 0 min == 0 sec to
          59sec)
          ...
          * Rules :
            ...
            * For save
              * When start-up
                * We need to find the latest(nearest) one in the future and give
          to it
              * When normal running
                * We save the next index after the current one
          * example
            1. (only for start-up)know what time is it -> find the nearest one
          in the future -> save the variable
        ...
        */
        ...
        uint8_t *tempTaskPointer =
            uint8NumToCharArr(taskScheduleList[readTaskIndex]);

        int taskSetTimeMin = readTaskIndex;
        // check if need to run task now
        // if not cross day, change to sec and compare
        // if time is not actually exceed the schdule, go sleep again, else is
        // keep running
...
//// print status and is need to run or not
            ...
            Serial.println("--- now : " +
                           secMapTo24Hour(getPassedSecOfToday()));
            Serial.println("--- readTaskIndex : " + String(readTaskIndex) +
                           "(" + minConvertTohour24(readTaskIndex) + ")");
            Serial.println("--- tasks : " +
                           uint8ToString(taskScheduleList[readTaskIndex]));
            Serial.println("--- execute : [True], now is exceed the set time.");

            writeMsgToPath(sensorDataPath, "- Execute status of this loop : ");
            writeMsgToPath(
                sensorDataPath,
                "--- now : " + secMapTo24Hour(getPassedSecOfToday()));
            writeMsgToPath(sensorDataPath,
                           "--- readTaskIndex : " + String(readTaskIndex) +
                               "(" + minConvertTohour24(readTaskIndex) + ")");
            writeMsgToPath(sensorDataPath,
                           "--- tasks : " +
                               uint8ToString(taskScheduleList[readTaskIndex]));
            writeMsgToPath(sensorDataPath,
                           "--- execute : [True], now is exceed the set time.");
#endif
            ...
            if (tempTaskPointer[TASK_A]) {
                ...
                isRunningTask += 1;
                // print and writr log
                Serial.println("----- " + String(today) + "_" +
                               String(secMapTo24Hour(getPassedSecOfToday())) +
                               " : run Task A (sound record)." +
                               " Set time : " + String(taskSetTimeMin) + "(" +
                               String(minConvertTohour24(taskSetTimeMin)) +
                               ")");
                writeMsgToPath(systemLogPath,
                               "----- Run Task A (sound record), set time : " +
                                   String(taskSetTimeMin) + "(" +
                                   String(minConvertTohour24(taskSetTimeMin)) +
                                   ")");
                // set global variable
            ...
            if (tempTaskPointer[TASK_B]) {
                ...
                Serial.println("----- " + String(today) + "_" +
                               ...
                               String(minConvertTohour24(taskSetTimeMin)) +
                               ")");
                DHT_TimeStamp = String(today) + "_" +
                                String(secMapTo24Hour(getPassedSecOfToday()));
                writeMsgToPath(systemLogPath,
                               "----- Run Task B (DHT), set time : " +
                                   String(taskSetTimeMin) + "(" +
                                   String(minConvertTohour24(taskSetTimeMin)) +
                                   ")",
                               DHT_TimeStamp);  // write with same timestamp
                                                // before/after get the data
                // active the suspended task
            ...
            // swift to next task until have task in that index and check it is
            // cross day or not
            //// it have auto re-zero when overflow
...
//// print status and is need to run or not
            ...
            Serial.println("--- tasks : " +
                           uint8ToString(taskScheduleList[readTaskIndex]));
            Serial.println(
                "--- execute : [False], now is before the set time.");
        ...
        if (isCrossDay) {
            ...
            nextTaskPreserveTime_sec =
                SECONDS_OF_A_DAY - getPassedSecOfToday() + taskSetTimeMin * 60;
        } else {
            // normal
            nextTaskPreserveTime_sec =
                taskSetTimeMin * 60 - getPassedSecOfToday();
        }
        Serial.println("----- sleepingTimeUntilNextTask_sec : " +
                       String(nextTaskPreserveTime_sec) + "\n");
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### checkTimeAndTask stack : " +
                       String(uxHighWaterMark) + "(byte) ####");
        writeMsgToPath(systemLogPath, "#### checkTimeAndTask stack : " +
                                          String(uxHighWaterMark) +
                                          "(byte) ####");
#endif
    }
        ...
        if (ulNotificationValue > 0) {
            ...
            if (xSemaphoreTake(xSemaphore_SD, portMAX_DELAY) == pdTRUE) {
                if (!soundFile.open(
                        _filenameWithPath,
                        O_WRONLY | O_CREAT |
                            O_APPEND)) {  // open need char array, not string.
                                          // So use c_str to convert
                    Serial.println(" --> open file failed, transmitToSD");
                    continue;
                }
                soundFile.write((uint8_t *)transmitAudioBuffer,
                                globalSDBufferByteSize);
                soundFile.close();
            }
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### transmitSoundDataToSD stack : " +
                       String(uxHighWaterMark) + "(byte) ####");
        writeMsgToPath(systemLogPath, "#### transmitSoundDataToSD stack : " +
                                          String(uxHighWaterMark) +
                                          "(byte) ####");
#endif
    }
...
#endif
        String recordPath = "/" + String(today) + "/" + String(today) + "_" +
                            String(secMapTo24Hour(getPassedSecOfToday()) + "_" +
                                   String(channel_tag) + ".wav");
        char filename[40];
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### recordSound stack : " + String(uxHighWaterMark) +
                       "(byte) ####");
        writeMsgToPath(systemLogPath,
                       "#### recordSound stack : " + String(uxHighWaterMark) +
                           "(byte) ####");
#endif
    }
}
...
// try to power on the sensor
        ...
        while (true) {
            ...
            if (xSemaphoreTake(xSemaphore_Temp_PMOS, pdMS_TO_TICKS(100)) ==
                pdTRUE) {
                turnOnDhtPower();
        ...
        // write SD
        writeMsgToPath(sensorDataPath,
                       "DHT temperature : " + String(temperature) + " C",
                       DHT_TimeStamp);
        writeMsgToPath(sensorDataPath,
                       "DHT Humidity : " + String(humidity) + " %",
                       DHT_TimeStamp);
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### recordDHT stack : " + String(uxHighWaterMark) +
                       "(byte) ####");
        writeMsgToPath(systemLogPath,
                       "#### recordDHT stack : " + String(uxHighWaterMark) +
                           "(byte) ####");
#endif
    }
}
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### recordDS18B20 stack : " + String(uxHighWaterMark) +
                       "(byte) ####");
        writeMsgToPath(systemLogPath,
                       "#### recordDS18B20 stack : " + String(uxHighWaterMark) +
                           "(byte) ####");
#endif
    }
}
...
// read part
        ...
        Serial.println("[Battery status : " + String(getBatteryVoltage()) +
                       "v (" + String(getBatteryPercentage()) + " %)]");
#endif
        writeMsgToPath(sensorDataPath,
                       "Battery status : " + String(getBatteryVoltage()) +
                           "v (" + String(getBatteryPercentage()) + "%)",
                       Battery_TimeStamp);
...
#ifdef STACK_DEBUG
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.println("#### recordBattery stack : " + String(uxHighWaterMark) +
                       "(byte) ####");
        writeMsgToPath(systemLogPath,
                       "#### recordBattery stack : " + String(uxHighWaterMark) +
                           "(byte) ####");
#endif
    }
}

// A function to check day change. If happened, create new folder and new log
// file Not a full RTOS task, we only use vTaskDelay to prevent blocking
void checkDayChange() {
    ...
    if (getPassedSecOfToday() > SECONDS_OF_A_DAY) {
        ...
        // if local time && RTC time both cross day (normal situation) or RTC
        // cross day but local not 1hr tolerance range of rtc time
        if (tempRtcTime < 3600) {
            sys_RTC_time_offset = tempRtcTime;
            sys_millis_time_offset =
                millis();  // notice : millis only can count 49 days
            Serial.println("day changed");
            // Write log
            writeMsgToPath(systemLogPath, "===> Day changed ", "", false,
                           false);
...
void goToSleep(int sleepTime_sec) {
    ...
    int canSleepMs =
        sleepTime_sec *
        1000;  // Serial.println("canSleepMs : " + String(canSleepMs));
    if (canSleepMs > 100) {
...
#endif
#endif
        Serial.println("--- Go to sleep for : " + String(canSleepMs / 1000.0f) +
                       " sec");
        writeMsgToPath(
            systemLogPath,
            "Go to sleep for : " + String(canSleepMs / 1000.0f) + " sec");
...
// debug MSG part
            ...
            Serial.println(String(secMapTo24Hour(getPassedSecOfToday())));
#endif
            Serial.println("Have slept for : " + String((i + 1)) +
                           " min. Wake up to feed dog.");
#endif
...
// short sleep part
        ...
        Serial.println(String(secMapTo24Hour(getPassedSecOfToday())));
#endif
        Serial.println("--- Sleep for remain " + String(remainMs / 1000.0f) +
                       " seconds\n");
#endif
        vTaskDelay(40 / portTICK_PERIOD_MS);
